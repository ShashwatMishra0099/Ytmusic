...
============================================================
  ...
  (Confirmed working via yt_debug.py on this VPS)

  Cache: cache/index.json maps video_id -> local mp3 path.
  Avoids re-downloading the same song twice.

  Pause/resume: uses calls.mute() / calls.unmute() as a
  workaround — the actual method name is determined at
  startup from introspection and filled into PAUSE_METHOD /
  RESUME_METHOD. Replace with correct names once confirmed
  via check_pytgcalls.py.
============================================================
"""
...
SESSION_STR   = (
    "BQGtxF0ACMlQJ4Yp2QkynXYhXKVV5sTwW7m2wKdgi5jjj8YZ0q7SrcL1L7JWcM4-b54y"
    "79YA3kLKUr_XoApnnNA_9m0Kcl_Bb9jf19zxmurZlcdZbB_hIDtR7aqxDMDjJGNbhTqwC"
    "zDk0voKOiRGncIFF0R_Ch1H9YDN9ZcmagWBvCfycMP4QbJlae5lLmkGnNsk8R--zVRhhlZ"
    "w0voGvlTESskkF_fbYYaN7UomTRsMPIE8IIh6Rr2FJeAR1MP9SZagejPC49ma1RB4ODfTM"
    "iYtI7S1FpvRnhbf8k4YMQjcQBWVQAi6Y0NbHt-KiAhrTcxbKpnh02acbfT76zQqCzkkmT"
    "qhaAAAAAHSbKT2AA"
)

SILENCE_WAV = "silence.wav"
CACHE_DIR   = "cache"               # folder where mp3s are stored
CACHE_INDEX = "cache/index.json"    # video_id -> {"file": path, "title": ..., ...}
...
def _cache_lookup(video_id: str) -> dict | None:
    """Return cached info dict if file exists, else None."""
    index = _load_cache()
...
def _ytdlp_download_sync(query: str) -> dict:
    ...
    1. Extracts video info WITHOUT downloading first to get video_id.
    2. Checks cache — returns cached entry immediately if hit.
    ...
    # Step 1: extract info only (no download) to get video_id
...
# ══════════════════════════════════════════════════════════
#  PAUSE / RESUME — METHOD NAME DETECTION
#
#  py-tgcalls has renamed these methods across versions:
#    v0.9.x  : pause_stream / resume_stream
#    v1.x+   : pause        / resume
#    some    : mute         / unmute  (for silencing)
#  We detect at runtime which names actually exist.
# ══════════════════════════════════════════════════════════
def _find_method(obj, *candidates):
    """Return the first method name from candidates that exists on obj."""
...
# These are filled in during main() after calls is created
_PAUSE_METHOD:  str | None = None
_RESUME_METHOD: str | None = None
...
# ══════════════════════════════════════════════════════════
#  /play  — ── DO NOT CHANGE DOWNLOAD LOGIC ──
# ══════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════
#  /pause  — calls correct method detected at startup
# ══════════════════════════════════════════════════════════
async def cmd_pause(message: Message) -> None:
    ...
    if _paused:
        ...
        return
    if not _PAUSE_METHOD:
        await message.reply(
            "Pause not supported on this version of py-tgcalls.\n"
            "Run check_pytgcalls.py and share the output.")
        return
...
# ══════════════════════════════════════════════════════════
#  /resume  — calls correct method detected at startup
# ══════════════════════════════════════════════════════════
async def cmd_resume(message: Message) -> None:
    ...
    if not _paused:
        ...
        return
    if not _RESUME_METHOD:
        await message.reply(
            "Resume not supported on this version of py-tgcalls.\n"
            "Run check_pytgcalls.py and share the output.")
        return
...
async def main() -> None:
    ...
    global _PAUSE_METHOD, _RESUME_METHOD
    ...
    # 4. Pause/resume method names — confirmed via check_pytgcalls.py
    #    py-tgcalls v2.2.11 uses: calls.pause(chat_id) / calls.resume(chat_id)
    _PAUSE_METHOD  = "pause"
    _RESUME_METHOD = "resume"
    logger.info("Pause method  : %s", _PAUSE_METHOD)
    logger.info("Resume method : %s", _RESUME_METHOD)
    ...
    # 6. PyTgCalls update handler  — ── DO NOT CHANGE ──
    @calls.on_update()
